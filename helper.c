...
char check_for_moves(Grid *g)
{
    ...
    for (int i = 0; i < 12; i++)
    {
        if (g->v[ind[i] - 1] == g->v[ind[i]] && g->v[ind[i]] != 0)
        { //merge
            return 'l';
        }
    }
    ...
    for (int i = 11; i >= 0; i--)
    {
        if (g->v[rind[i] + 1] == g->v[rind[i]] && g->v[rind[i]] != 0)
        { //merge
            return 'r';
        }
    }
...
void initscreen()
{
    ...
    wchar_t c = '#'; // 0x2593;
    wprintf(L"" YEL BOLD);
    // wprintf(L"\t %lc%lc%lc%lc%lc    %lc%lc%lc   %lc        %lc%lc%lc%lc%lc\n", c, c, c, c, c, c, c, c, c, c, c, c, c, c);
...
void setgrid(Grid *g, char com)
{
    ...
    case 'u':
    {
        for (int i = 4; i < 16; i++)
        {
            if (g->v[i - 4] == 0 && g->v[i] != 0)
            { //shift
                change = 1;
                g->v[i - 4] = g->v[i];
                g->v[i] = 0;
                if (i - 4 >= 4)
                    i -= 5; //5 to correct for the ++, I should ideally use continue;
            }
            else if (g->v[i - 4] == g->v[i] && g->v[i] != 0)
            { //merge & shift
                change = 1;
                g->v[i - 4] += g->v[i];
                g->v[i] = 0;
    ...
    case 'l':
    {
        ...
        for (int i = 0; i < 12; i++)
        {
            if (g->v[ind[i] - 1] == 0 && g->v[ind[i]] != 0)
            { //shift
                change = 1;
                g->v[ind[i] - 1] = g->v[ind[i]];
                g->v[ind[i]] = 0;
                if (i - 1 >= 0 && ind[i] % 4 != 1)
                    i -= 2; //-2 to compensate for ++
            }
            else if (g->v[ind[i] - 1] == g->v[ind[i]] && g->v[ind[i]] != 0)
            { //merge
                change = 1;
                g->v[ind[i] - 1] += g->v[ind[i]];
                g->v[ind[i]] = 0;
    ...
    else if (change == 1)
    {
        ...
        int randn = 2;
        if (rand() % 10 == 0)
            randn = 4; //10% chance of 4, 90% chance of 2, no chance of 0
